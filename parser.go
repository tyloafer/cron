...
func Every(duration time.Duration) Parser {
...
func (schedule Parser) Next(t time.Time) time.Time {
	fmt.Println(schedule.Delay)
	os.Exit(-1)
...
func Parse(spec string) (_ ParserInterface, err error) {
...
// if none matches.
func parseDescriptor(spec string, loc *time.Location) ParserInterface {
	switch spec {
